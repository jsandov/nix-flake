...
- **sops-nix master assumes bleeding-edge nixpkgs.** On 2026-02-04 sops-nix bumped `sops-install-secrets` to `buildGo125Module` (removed 24.11/25.05 compatibility in the same commit). Our flake follows `nixos-24.11` nixpkgs which ships Go 1.24, so `github:Mic92/sops-nix` (defaulting to master) broke the build with `Function called without required argument "buildGo125Module"`. Fix: pin the sops-nix input to a commit SHA from before the Go 1.25 switch — `3b4a369df9` (2025-09-09) is known good. Revisit the pin when nixpkgs moves to 25.05+. Lesson reinforced: never `url = "github:owner/repo"` without a rev for a flake input that gates the build.
- **`gh run watch --exit-status` does not always reflect run conclusion.** After the fix commit, `gh run watch <id> --exit-status` exited 0 even though the watched run had `conclusion: FAILURE`. Suspected cause: the push triggered more than one workflow run in quick succession, and `watch` followed a different run than the failing one, or the GitHub Actions API is eventually-consistent about the final status. Always double-check `gh pr view <n> --json statusCheckRollup` before merging, regardless of what watch returns. We merged a broken PR because of this and had to push a hotfix (PR #27).
